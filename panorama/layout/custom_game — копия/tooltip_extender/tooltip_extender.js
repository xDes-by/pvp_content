...
function RegisterHoverableAbility(data) {
	registered_abilities[data.loc_name.toUpperCase()] = data.raw_name;
}
...
function TooltipTextChanged(data) {
	...
	const ad_scepter_localized = ad_scepter_loc_token != ad_scepter_loc_string;
...
function PortraitUnitChanged() {
	...
	if (!unit || !Entities.IsRealHero(unit)) {
		return;
	}
	for (i = 0; i < Entities.GetAbilityCount(unit); i++) {
		...
		if (ability && ability != -1 && !Abilities.IsHidden(ability)) {
			...
			RegisterHoverableAbility({
				loc_name: $.Localize("#DOTA_Tooltip_ability_" + ability_name).toUpperCase(),
				raw_name: ability_name,
			});
		}
	}
}
...
(function () {
	$.Schedule(2, function () {
		GameEvents.Subscribe("AbilityNotes:register", RegisterHoverableAbility);
